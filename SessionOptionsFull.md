# SMKitUI Android 1.9.1 session options
...
Android 1.9.1 exposes four built-in assessment categories:
...
Android 1.9.1 publishes English and Hebrew. Pause-menu labels follow the selected session language, independent of the device locale.
...
The demo's fresh-install skeleton state mirrors the iOS demo: visible, `DEFAULT`, `SOLID`, `CIRCLE`, preset colors, full opacity, no glow or softness, 1× line and outline scales, and zero animation duration. The controls below can then be used to demonstrate a customized style:
...
`SMExercise` exposes the following Android 1.9.1 options. `MainViewModel.kt` populates the catalog-driven options; `guidanceVideoSegments` remains available for host-provided video timelines:
...
Every setting in this document maps to a public Android 1.9.1 type or method and is represented directly in the demo without reflection or placeholder controls.
