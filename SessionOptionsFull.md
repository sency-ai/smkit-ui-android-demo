# SMKitUI Android 1.9.3 session options
...
Android 1.9.3 exposes four built-in assessment categories:
...
Android 1.9.3 publishes English and Hebrew. Pause-menu labels follow the selected session language, independent of the device locale.
...
`SMExercise` exposes the following Android 1.9.3 options. `MainViewModel.kt` populates the catalog-driven options; `guidanceVideoSegments` remains available for host-provided video timelines:
...
Every setting in this document maps to a public Android 1.9.3 type or method and is represented directly in the demo without reflection or placeholder controls.
